...
for k = 1:length(t)-1
    ...
    dx2 = (-a.*(x(k)+dx1/2)+b.*(1+sin(omega.*(t1))))*T;
    dx3 = (-a.*(x(k)+dx2/2)+b.*(1+sin(omega.*(t1))))*T;
    dx4 = (-a.*(x(k)+dx3)+b.*(1+sin(omega.*(tk+T))))*T;
...
title('Runge-Kutta method','Interpreter','latex','FontSize',12)
grid on; grid minor;
